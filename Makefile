...
.PHONY: create_venv # Create Python virtualenv under venv/
create_venv:
	virtualenv venv
...
setup_venv:
	...
	(echo "Virtualenv not available, please create one named 'venv' at the root of this repository." && exit 1 ) && \
...
test_web_js_api_parallel_chrome_ci: create_venv setup_venv
	source ~/.nvm/nvm.sh && \
	nvm install $(NODE_VERSION) && \
	nvm use $(NODE_VERSION) && \
...
bench_web_js_api_parallel_chrome_ci: create_venv setup_venv
	source ~/.nvm/nvm.sh && \
	nvm install $(NODE_VERSION) && \
	nvm use $(NODE_VERSION) && \
