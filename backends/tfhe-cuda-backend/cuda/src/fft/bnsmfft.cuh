...
template <class params> __device__ void NSMFFT_direct(double2 *A) {
  ...
  constexpr Index Z = params::opt >> 1;     // number of butterfly on each level
  constexpr Index LG = params::log2_degree; // log2 degree
  constexpr Index N = params::degree;       // Degree
  constexpr Index HN = params::degree >> 1; // half degree
  constexpr Index BS = params::degree / params::opt; // block size

  Index tid = threadIdx.x;
  Index twid_id;
  Index i1, i2;
  double2 u[Z], v[Z], w, root;
...
#pragma unroll
    for (Index i = 0; i < Z; i++) {
      ...
      w = negtwiddles[tid / lane_mask + twid_shift];
...
template <class params> __device__ void NSMFFT_inverse(double2 *A) {
  ...
  constexpr Index Z = params::opt >> 1;
  constexpr Index LG = params::log2_degree;
  constexpr Index N = params::degree;
  constexpr Index HN = params::degree >> 1;
  constexpr Index BS = params::degree / params::opt;
...
#pragma unroll
  ...
  Index twid_shift = params::degree;
  for (Index l = 1; l <= LG - 1; ++l) {
    Index lane_mask = 1 << (l - 1);
    Index thread_mask = (1 << l) - 1;
    tid = threadIdx.x;
    twid_shift >>= 1;
...
#pragma unroll
    for (Index i = 0; i < Z; ++i) {
      w = (u[i] - v[i]);
      u[i] += v[i];
      v[i] = w * conjugate(negtwiddles[tid / lane_mask + twid_shift]);
