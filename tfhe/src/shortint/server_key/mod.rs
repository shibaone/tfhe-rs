...
impl ParameterSetConformant for ServerKey {
    ...
    fn is_conformant(&self, parameter_set: &Self::ParameterSet) -> bool {
        ...
        let expected_max_degree = MaxDegree::from_msg_carry_modulus(
            parameter_set.message_modulus(),
            parameter_set.carry_modulus(),
        );

        let pbs_order_ok = match (*pbs_order, parameter_set.encryption_key_choice()) {
            (PBSOrder::KeyswitchBootstrap, EncryptionKeyChoice::Big)
            | (PBSOrder::BootstrapKeyswitch, EncryptionKeyChoice::Small) => true,
            (PBSOrder::BootstrapKeyswitch, EncryptionKeyChoice::Big)
            | (PBSOrder::KeyswitchBootstrap, EncryptionKeyChoice::Small) => false,
        };
